...
dragcfact(const Arg *arg)
{
	...
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
		None, cursor[CurResizeTL]->cursor, CurrentTime) != GrabSuccess)
		return;
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - stw - x) > bh) {
		if (n > 0) {
			stw = TEXTW(m->sel->name) + lrpad;
			mw = (stw >= w || n == 1) ? 0 : (w - stw) / (n - 1);
			...
			for (c = m->clients; c; c = c->next) {
				if (!ISVISIBLE(c))
					continue;
				stw = MIN(m->sel == c ? w : mw, TEXTW(c->name));

				drw_setscheme(drw, scheme[m->sel == c ? SchemeSel : SchemeNorm]);
				if (stw > 0) /* trap special handling of 0 in drw_text */
					drw_text(drw, x, 0, stw, bh, lrpad / 2, c->name, 0);
				if (c->isfloating)
